...
extends PhabricatorConfigOptionType {
  ...
  public function getDisplayValue(
    ...
    $phid_to_callsign = array();
    foreach ($repositories as $repository) {
      $phid_to_callsign[$repository->getPHID()] = $repository->getCallsign();
    }

    $callsigns = array();
    foreach ($value as $phid) {
      if (isset($phid_to_callsign[$phid])) {
        $callsigns[] = $phid_to_callsign[$phid];
      } else {
        $callsigns[] = pht('<not found: %s>', $phid);
      }
    }
